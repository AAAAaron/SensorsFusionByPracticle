...
public:
	...
	double wifi_weight_unit;//一个单元权值增量
...
public:
	...
	map<string,vector<vector<double> > > wifi_rssi_fp;//重新构建wifi矩阵
	vector<string> region_list;//内建的区域列表
	map<string ,Practicle> practicle_map;//维护的粒子对象，多个,外部检测对象是否重复，重复直接覆盖
	...
	Point main_progration(string prac_name,int step_index,string floor_id, double step_length,double step_deta_angle,double Heading, double mag_norm,double mag_z,double mag_y,double inclination);//地磁空间
	Point main_progration(string prac_name,int step_index,string floor_id,double step_length,double step_deta_angle,double Heading, vector<wifi_match_result> wifidata_list);//用于wifi信号的重载
